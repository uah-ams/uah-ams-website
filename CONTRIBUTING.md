...
This is the website for the UAH AMS/NWA, and is developed with standard Github flow.
...
Discover a bug?
Want a new feature?
[Open](https://github.com/uah-ams/uah-ams-website/issues/new/choose) an [issue](https://github.com/uah-ams/uah-ams-website/issues)!
...
First, navigate to the [project repository](https://github.com/UAH-AMS/uah-ams-website) in your web browser and press the ``Fork`` button to make a personal copy of the repository on your own Github account.
...
```bash
git remote add upstream https://github.com/UAH-AMS/uah-ams-website.git
```
...
```bash
conda create -c conda-forge -n uah-ams-website-env python=3
```

And activate it with

```bash
conda activate uah-ams-website-env
```

With pip/venv, you can create a virtual environment with

```bash
python -m venv uah-ams-website-env
```

And activate it with the following on Linux and macOS,

```bash
source uah-ams-website-env/bin/activate
```

or on Windows (cmd),

```cmd
.\uah-ams-website-env\Scripts\activate.bat
```
...
Finally, create a pull request to the [uah-ams/uah-ams-website repository](https://github.com/uah-ams/uah-ams-website/) on Github.
